...
contract BalancerTest is Test {
    ...
    function testBalancerPoolBalance() public {
        ...
        uint poolInvariant_ = pool.getInvariant();
        uint outputDecimals_ = 12;
        uint poolSupply_ = pool.totalSupply();

        uint poolMultiplier = poolInvariant_.mulDiv(10 ** outputDecimals_, poolSupply_);
...
//
//        // Balancer 80%, Weth 20%
//        (IERC20[] memory tokens, uint256[] memory balances, uint256 lastChangeBlock) = balancer.getPoolBalance(addr);
//
